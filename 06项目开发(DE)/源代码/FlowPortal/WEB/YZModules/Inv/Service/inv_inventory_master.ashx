...
namespace Inv
{
    public class inv_inventory_master : YZServiceHandler
    {
        ...
        public JObject GetData(HttpContext context)
        {
            ...
            {
                cn.WebOpen();
                moduleAdmin = BPM.Client.Security.UserResource.CheckPermision(cn, "aa016f07-c991-4803-ad17-e8c8f44dbcbd", "Admin");
                if (!moduleAdmin)
                {
                    bool moduleCompany = BPM.Client.Security.UserResource.CheckPermision(cn, "aa016f07-c991-4803-ad17-e8c8f44dbcbd", "Company");
                    if (moduleCompany)
                    {
