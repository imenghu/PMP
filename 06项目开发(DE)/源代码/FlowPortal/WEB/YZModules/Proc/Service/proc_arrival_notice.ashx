    ...
    public JObject GetData(HttpContext context)
    {
        ...
        if (searchType == "QuickSearch")
        {
            ...
            if (!string.IsNullOrEmpty(keyword))
                filter = queryProvider.CombinCond(filter, String.Format("CompanyName LIKE N'%{0}%' or mat_name LIKE N'%{0}%' or vendor_name LIKE N'%{0}%' or StoreUserName LIKE N'%{0}%'", queryProvider.EncodeText(keyword)));
