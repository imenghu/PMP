<%@ WebHandler Language="C#" Class="proc_pur_task" %>
...
public class proc_pur_task : YZServiceHandler
{
    ...
    public JObject GetData(HttpContext context)
    {
        ...
        using (BPMConnection cn = new BPMConnection())
        {
            ...
            if (!moduleAdmin)
            {
                ...
                {
                    filter = queryProvider.CombinCond(filter, string.Format("TaskUser='{0}'", YZAuthHelper.LoginUserAccount));
                }
            }
        }
        ...
        if (searchType == "QuickSearch")
        {
            ...
            if (!string.IsNullOrEmpty(keyword))
                filter = queryProvider.CombinCond(filter, String.Format("CompanyName LIKE N'%{0}%' or deptname LIKE N'%{0}%' or mat_name LIKE N'%{0}%' or TaskUserName LIKE N'%{0}%'", queryProvider.EncodeText(keyword)));
