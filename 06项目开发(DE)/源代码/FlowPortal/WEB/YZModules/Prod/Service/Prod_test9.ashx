...
namespace Prod
{
    public class Prod_test9 : YZServiceHandler
    {
        public void Delete(HttpContext context)
        {
            ...
            using (SqlConnection cn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SFDATA"].ConnectionString))
            {
                ...
                foreach (int id in ids)
                {
                    ...
                    cmd.Connection = cn;
                    cmd.CommandText = "update Prod_test9 set state = '0' WHERE test_id=@id";
        ...
        public JObject GetData(HttpContext context)
        {
            ...
            {
                cn.WebOpen();
                moduleAdmin = BPM.Client.Security.UserResource.CheckPermision(cn, "dd82286f-627e-4312-89f1-c396e5489b01", "Admin");
                if (!moduleAdmin)
                {
                    bool moduleCompany = BPM.Client.Security.UserResource.CheckPermision(cn, "dd82286f-627e-4312-89f1-c396e5489b01", "Company");
                    if (moduleCompany)
                    {
            ...
            if (searchType == "QuickSearch")
            {
                ...
                if (!string.IsNullOrEmpty(keyword))
                    filter = queryProvider.CombinCond(filter, String.Format("CompanyName LIKE N'%{0}%' or mat_name LIKE N'%{0}%' or hops_type LIKE N'%{0}%' ", queryProvider.EncodeText(keyword)));
