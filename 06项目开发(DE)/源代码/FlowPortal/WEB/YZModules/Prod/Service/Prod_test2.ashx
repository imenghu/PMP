...
namespace Prod
{
    public class Prod_test2 : YZServiceHandler
    {
        public void Delete(HttpContext context)
        {
            ...
            using (SqlConnection cn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SFDATA"].ConnectionString))
            {
                ...
                foreach (int id in ids)
                {
                    ...
                    cmd.Connection = cn;
                    cmd.CommandText = "update Prod_test2 set state = '0' WHERE test_id=@id";
        ...
        public JObject GetData(HttpContext context)
        {
            ...
            {
                cn.WebOpen();
                moduleAdmin = BPM.Client.Security.UserResource.CheckPermision(cn, "ef2acf37-e479-4936-a7af-1e87a7d999ed", "Admin");
                if (!moduleAdmin)
                {
                    bool moduleCompany = BPM.Client.Security.UserResource.CheckPermision(cn, "ef2acf37-e479-4936-a7af-1e87a7d999ed", "Company");
                    if (moduleCompany)
                    {
