<%@ WebHandler Language="C#" Class="Prod.prod_test1" %>
...
namespace Prod
{
    public class prod_test1 : YZServiceHandler
    {
        public void Delete(HttpContext context)
        {
            ...
            using (SqlConnection cn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SFDATA"].ConnectionString))
            {
                ...
                foreach (int id in ids)
                {
                    ...
                    cmd.Connection = cn;
                    cmd.CommandText = "update prod_test1 set state = '0' WHERE test_id=@id";
        ...
        public JObject GetData(HttpContext context)
        {
            ...
            {
                cn.WebOpen();
                moduleAdmin = BPM.Client.Security.UserResource.CheckPermision(cn, "08e56d79-57d9-4a26-a48c-47a3d91a9e44", "Admin");
                if (!moduleAdmin)
                {
                    bool moduleCompany = BPM.Client.Security.UserResource.CheckPermision(cn, "08e56d79-57d9-4a26-a48c-47a3d91a9e44", "Company");
                    if (moduleCompany)
                    {
            ...
            string query = @"
            WITH X AS(
                SELECT ROW_NUMBER() OVER(ORDER BY {0}) AS RowNum,* FROM prod_test1 {1}
            ),
            Y AS(
                                    ...
                                    reader.ReadInt32("test_id");
                                                            item["CompanyName"] = 
                                                                    ...
                                                                    reader.ReadString("operator_name");
                                                                                            item["sample_date"] = 
                                                                    reader.ReadDateTime("sample_date");
                                                                                    }
