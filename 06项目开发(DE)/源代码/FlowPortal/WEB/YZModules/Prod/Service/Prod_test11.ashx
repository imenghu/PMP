<%@ WebHandler Language="C#" Class="Prod.Prod_test11" %>
...
namespace Prod
{
    public class Prod_test11 : YZServiceHandler
    {
        public void Delete(HttpContext context)
        {
            ...
            using (SqlConnection cn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["SFDATA"].ConnectionString))
            {
                ...
                foreach (int id in ids)
                {
                    ...
                    cmd.Connection = cn;
                    cmd.CommandText = " update Prod_test11 set state ='0' WHERE test_id=@id";
        ...
        public JObject GetData(HttpContext context)
        {
            ...
            {
                cn.WebOpen();
                moduleAdmin = BPM.Client.Security.UserResource.CheckPermision(cn, "865a3261-dcd6-4f99-8739-f591eed7837d", "Admin");
                if (!moduleAdmin)
                {
                    bool moduleCompany = BPM.Client.Security.UserResource.CheckPermision(cn, "865a3261-dcd6-4f99-8739-f591eed7837d", "Company");
                    if (moduleCompany)
                    {
            ...
            string query = @"
            WITH X AS(
                SELECT ROW_NUMBER() OVER(ORDER BY {0}) AS RowNum,* FROM Prod_test11 {1}
            ),
            Y AS(
                            ...
                            item["test_id"] =
                                                            ...
                                                            item["CompanyName"] = 
                                                                    ...
                                                                    reader.ReadString("spec");
                                                                                            item["smell"] = 
                                                                    reader.ReadString("smell");
                                                                                            item["purity"] = 
                                                                    reader.ReadString("purity");
                                                                                            item["memo"] = 
