<%@ WebHandler Language="C#" Class="Sys.ctl_depot" %>
...
namespace Sys
{
    public class ctl_depot : YZServiceHandler
    {
        ...
        public JObject GetData(HttpContext context)
        {
            ...
            {
                cn.WebOpen();
                moduleAdmin = BPM.Client.Security.UserResource.CheckPermision(cn, "4d29323a-a1b2-4454-a3f5-7098689cbebd", "Admin");
                if (!moduleAdmin)
                {
                    bool moduleCompany = BPM.Client.Security.UserResource.CheckPermision(cn, "4d29323a-a1b2-4454-a3f5-7098689cbebd", "Company");
                    if (moduleCompany)
                    {
            ...
            //获得排序子句
            string order = request.GetSortString("depot_id");

            //获得Query
            string query = @"
            WITH X AS(
                SELECT ROW_NUMBER() OVER(ORDER BY {0}) AS RowNum,* FROM ctl_depot {1}
            ),
            Y AS(
